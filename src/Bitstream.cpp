...
//We'll link them all together, then arbitrarily push them into 'chip''s appropriate BArrays.
	...
	std::map<std::string, Wiring*>::iterator Iwires;

	std::map<std::string, Gate*> gates;	
	std::map<std::string, Gate*>::iterator Igates;
...
void Hardware::readWord(char * c, size_t pos, std::string &s)
{
	int i = 0;
	while(c[pos] != ' ')
	{
...
//When parsing you may or may not then want to move the pos forward, do this in parseHDL(...) after calling this.
}
